...
namespace Math
{
    ...
    public:
        ...
        AGZ_FORCE_INLINE bool operator==(const Self &rhs) { return x == rhs.x && y == rhs.y && z == rhs.z; }
        AGZ_FORCE_INLINE bool operator!=(const Self &rhs) { return x != rhs.x || y != rhs.y || z != rhs.z; }
    ...
    template<typename T1, typename T2> AGZ_FORCE_INLINE
        auto operator+(const T1 &lhs, const Vec3<T2> &rhs) { return Vec3<decltype(lhs + rhs.x)>(lhs + rhs.x, lhs + rhs.y, lhs + rhs.z); }
    template<typename T1, typename T2> AGZ_FORCE_INLINE
        auto operator+(const Vec3<T1> &lhs, const T2 &rhs) { return Vec3<decltype(lhs.x + rhs)>(lhs.x + rhs, lhs.y + rhs, lhs.z + rhs); }
    template<typename T1, typename T2> AGZ_FORCE_INLINE
        auto operator-(const Vec3<T1> &lhs, const T2 &rhs) { return Vec3<decltype(lhs.x - rhs)>(lhs.x - rhs, lhs.y - rhs, lhs.z - rhs); }
    template<typename T1, typename T2> AGZ_FORCE_INLINE
        auto operator*(const T1 &lhs, const Vec3<T2> &rhs) { return Vec3<decltype(lhs * rhs.x)>(lhs * rhs.x, lhs * rhs.y, lhs * rhs.z); }
