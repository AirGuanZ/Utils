...
template<>           struct Abs_impl<float>  { static AGZ_FORCE_INLINE float Abs(float v) { return std::abs(v); } };
template<>           struct Abs_impl<double> { static AGZ_FORCE_INLINE double Abs(double v) { return std::fabs(v); } };
...
template<>           AGZ_FORCE_INLINE float  Sqrt<float>(float value)   { return std::sqrt(value); }
